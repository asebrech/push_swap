...
/*   Updated: 2021/06/12 12:58:11 by asebrech         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((*a)->next && (*a)->nb > (*a)->next->nb && intab((*a), tab, len) && intab((*a)->next, tab , len))
		ft_swap(a, 'a');
}

static void	checkthree(t_list **a, t_list **b, int *tab, int len)
{
	if ((*a)->next && (*a)->next->next && intab((*a)->next, tab, len) && intab((*a)->next->next, tab, len) && (*a)->next->nb > (*a)->next->next->nb)
	{
		ft_push(b, a, 'b');
		ft_swap(a, 'a');
		ft_push(a, b, 'a');
...
{
	if ((*a)->next && (*a)->next->nb > b->nb && intab((*a)->next, tab, len))
		ft_swap(a, 'a');
}
